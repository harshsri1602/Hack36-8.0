...
const HomePage: NextPage = () => {
    ...
    const PRIORITY_BORDER_COLORS: Record<Priority, string> = {
        Low: "border-green-600 hover:border-green-400 hover:shadow-[0_0_15px_rgba(34,197,94,0.8)]",
        Medium: "border-blue-600 hover:border-blue-400 hover:shadow-[0_0_15px_rgba(59,130,246,0.8)]",
        High: "border-yellow-500 hover:border-yellow-300 hover:shadow-[0_0_15px_rgba(234,179,8,0.8)]",
        Critical: "border-red-600 hover:border-red-400 hover:shadow-[0_0_15px_rgba(220,38,38,0.8)]",
        None: "border-gray-400 hover:border-gray-300 hover:shadow-[0_0_15px_rgba(156,163,175,0.8)]",
    };

    const badgePrio = (num: number) => {
        return num === 0 ? "Low" : num === 1 ? "Medium" : num === 2 ? "High" : num === 3 ? "Critical" : "Low";
    };

    const PrioVote = (num: number) => {
        return num === 0 ? "Low" : num === 1 ? "Medium" : num === 2 ? "High" : num === 3 ? "Critical" : "None";
    };
        ...
        const fetchRegionPosts = async () => {
            try {
                ...
                for (let i = 0; i < data.posts.length; i++) {
                    ...
                    const num = temp.lowCount + temp.mediumCount + temp.highCount + temp.criticalCount || 1;
                    temp.weightedSeverity = Math.round(
                        (temp.lowCount * 0 + temp.mediumCount * 1 + temp.highCount * 2 + temp.criticalCount * 3) / num
                    );
                        ...
                        {posts.map((post, index) => (
                            <div
                                ...
                                className={`cursor-pointer border-2  ${
                                    PRIORITY_BORDER_COLORS[badgePrio(post.weightedSeverity || 0)]
                                } bg-[#1A1A1A] rounded-lg shadow-md hover:shadow-xl hover:animate-pulse-border transition-all duration-300 min-h-[25vh] flex flex-col justify-center`}
                            >
                                <motion.div
                                ...
                                >
                                    <div className="flex flex-row items-start rounded-lg p-4 gap-4">
                                        <div className="flex flex-col items-center justify-center space-y-2">
                                            <Button
                                                onClick={(e) => {
                                                    ...
                                                    const newVoteType = Math.min(
                                                        3,
                                                        post.currentUserVoted ? post.currentVoteType + 1 : 0
                                                    );
                                                ...
                                                className={`${
                                                    SEVERITY_COLORS[
                                                        PrioVote(post.currentUserVoted ? post.currentVoteType : -1)
                                                    ]
                                        ...
                                        <div className="flex-1">
                                            <h3 className="font-bold text-2xl mb-2 text-white/90">{post.title}</h3>
                                            <div className="flex justify-center mb-3">
                                                <ImageScroller images={post.images || []} />
                                            </div>
                                            <p className="text-gray-300 leading-relaxed mb-2">{post.description}</p>
